...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      // 浏览页上刚挑的那张：一行小提示 + 「用这张」，点了才进草稿。
      '.reel_localWall{display:flex;align-items:center;gap:8px;margin:6px 0 0;flex-wrap:wrap;color:var(--dsw-alias-label-secondary);font-size:12px;line-height:1.5}',
      '.reel_localWall code,.reel_localWall span{overflow-wrap:anywhere}',
    ...
    function ReelCard(props) {
      ...
      const currentWall = typeof value.wallpaper === 'string' ? value.wallpaper : ''
      const shownRoots = draft ? draft.roots : currentRoots
      const shownCache = draft ? draft.cacheDir : currentCache
      const shownWall = draft ? draft.wallpaper : currentWall
      ...
      // 浏览页上刚用 🖼 挑的那条是**本机**选择（同一个源下的 localStorage），配置里
      // 看不到它。这里读出来给个「用这张」：一点就填进草稿，保存后它才成为所有
      // 设备共用的默认。不自动写配置——用户没点保存，就不该动他的设置。
      ...
      const save = async () => {
        ...
        const nextWall = draft.wallpaper.trim()
        const ops = []
        ...
        if (nextWall !== currentWall) {
          ops.push(nextWall === ''
            ? { op: 'unset', path: ['wallpaper'] }
            : { op: 'set', path: ['wallpaper'], value: nextWall })
        }
        if (ops.length === 0) {
          setDraft(null)
          return
        ...
        open ? h('div', { className: 'reel_body' },
          ...
          h('div', { className: 'reel_field' },
              ...
              shownRoots.map((root, index) => h('li', { className: 'reel_rootRow', key: index },
                h('input', {
                  ...
                  onChange: (event) => {
                    ...
                    setDraft({ roots: list, cacheDir: shownCache, wallpaper: shownWall })
                  },
                }),
                h('button', {
            ...
            h('button', {
              ...
              onClick: () => setDraft({ roots: shownRoots.concat(''), cacheDir: shownCache, wallpaper: shownWall }),
            }, `＋ ${t('addRoot')}`),
          ),
          h('div', { className: 'reel_field' },
            h('div', { className: 'reel_head' },
            ...
            h('div', { className: 'reel_cacheRow' },
              h('input', {
                ...
                disabled,
                spellCheck: false,
                onChange: (event) => setDraft({ roots: shownRoots.slice(), cacheDir: event.target.value, wallpaper: shownWall }),
              }),
            ),
          ...
          h('div', { className: 'reel_field' },
            h('div', { className: 'reel_head' },
              h('span', { className: 'reel_label' }, t('wallpaper')),
              has('wallpaper') ? h('button', {
                type: 'button', className: 'reel_reset', disabled,
                title: t('overridden'), onClick: () => resetField('wallpaper'),
              }, t('reset')) : null,
            ),
            h('div', { className: 'reel_cacheRow' },
              h('input', {
                ...
                disabled,
                spellCheck: false,
                onChange: (event) => setDraft({ roots: shownRoots.slice(), cacheDir: shownCache, wallpaper: event.target.value }),
              }),
            ),
            h('p', { className: 'reel_hint' }, t('wallpaperHint')),
            shownWall === '' && localWallpaper !== ''
              ? h('p', { className: 'reel_localWall' },
                h('span', null, `${t('wallpaperLocal')}${localWallpaper}`),
                h('button', {
                  type: 'button', className: 'reel_reset', disabled,
                  onClick: () => setDraft({ roots: shownRoots.slice(), cacheDir: shownCache, wallpaper: localWallpaper }),
                }, t('useThis')),
              )
              : null,
          ),
