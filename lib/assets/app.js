...
(() => {
  ...
  /**
   * 点「▾」弹出的菜单：**只有子文件夹**，外加一个「本层及以下」。
   *
   * 导航（去这一层、去根）已经在路径行上了，菜单里再来一遍就是同一件事画两处。
   * 所以这里只放「从这一级往下走」的动作：递归看这一整个子树，以及挑一个子
   * 文件夹进去。
   *
  ...
  const openFolderMenu = async (anchor, crumb, options = {}) => {
    ...
    const recursive = button('crumb-menu-item', '', () => {
      closeNow()
      setScope(state.scope === 'all' ? 'dir' : 'all')
    })
    recursive.dataset.allEntry = '1'
    recursive.setAttribute('role', 'menuitemradio')
    recursive.setAttribute('aria-checked', String(state.scope === 'all'))
    const tick = document.createElement('span')
    tick.className = 'tick'
    tick.textContent = state.scope === 'all' ? '✓' : ''
    tick.setAttribute('aria-hidden', 'true')
    const recursiveLabel = document.createElement('span')
    recursiveLabel.className = 'crumb-menu-name'
    recursiveLabel.textContent = '本层及以下'
    recursiveLabel.title = `把「${crumb.name}」下面所有层级的媒体列在一起`
    recursive.append(tick, recursiveLabel)
    menu.appendChild(recursive)

    const rule = document.createElement('div')
    rule.className = 'crumb-menu-sep'
  ...
  const mountChunk = (entry) => {
    ...
    for (let index = entry.from; index < entry.to; index++) {
      fragment.appendChild(buildCard(state.media[index], index))
    }
   ...
   *
   * 两个范围都支持：
   *  · `本目录` —— 当前层的散文件归成第一组（组名用当前目录名，否则切到
   *    分组后这层文件就没了），之后一个子文件夹一张卡。空文件夹（服务端
   *    顺手数好了直接媒体数）连卡都不出；懒加载后仍空的组当场撤掉。
   *  · `全部` —— 按每条文件所在的目录分组，组从扫描结果里现成推导，不再
   *    发请求。所以「全部 + 分组」等于「每个目录一张卡，直接铺开」。
   *
   * 组内网格和平铺共用同一套列宽（--tile）：卡片的尺寸不随组变，组与组
   * 之间只差「卡片的多少」。
   *
   * @returns {boolean} 实际是否按分组渲染——没有子文件夹时回退成平铺，
   *   调用方要据此决定网格的 `is-grouped` 类，否则平铺块会套上分组的样式。
   */
  const renderGrouped = (host = el.grid) => {
    ...
    if (state.scope === 'all') {
      ...
      units = []
      // 本层自己的散文件也归成一组（组名用当前目录名），不然切到分组模式
      // 这一层的文件就从页面上消失了——顶层目录往往就挂着几百个散文件。
      if (state.media.length > 0) {
        const crumbs = listing.crumbs ?? []
        const here = crumbs.length > 0 ? crumbs[crumbs.length - 1].name : listing.root.label
        units.push({ key: listing.key, name: here, files: state.media })
      }
      for (const folder of groups) {
        // 服务端顺手数了每个子文件夹的直接媒体数：空文件夹连组都不建，
        // 用户不用滚过一串「没有文件」。mediaCount 为 null（没统计/读不了）
        // 时保守地照常显示。筛选（只看视频等）导致的空组由懒加载后移除兜底。
        if (folder.mediaCount === 0) continue
        units.push({ key: folder.key, name: folder.name, files: null })
      }
      if (units.length === 0) {
        // 这一层没有子文件夹也没有文件，分组没有意义，直接平铺。回退必须进
        // 同一个 host（fragment），并且把「实际是平铺」告诉调用方——网格上的
        // is-grouped 类决定分块样式，留着它平铺块就没有布局规则了。
        renderFlatGrid(host)
        return false
      }
    }
    ...
    for (const unit of units) {
      ...
      const paint = (items) => {
        clear(body)
        if (items.length === 0) {
          // 加载后一条都没有（多半是筛选/只看视频把内容滤光了）：整个组
          // 撤掉，别留一张空卡。要是撤完一组不剩，把空状态提示亮出来。
          placeholder?.disconnect()
          const parent = group.parentNode
          group.remove()
          if (parent !== null && parent.querySelectorAll('.folder-group').length === 0) {
      ...
      // 未加载：先占位，等它进入视野再读那一层。
      tally.textContent = '…'
      placeholder = new IntersectionObserver((entries) => {
        ...
        queue.push(async () => {
          ...
          const wanted = (result.payload.files ?? []).filter((item) =>
            state.kinds === 'video' ? item.kind === 'video'
              : state.kinds === 'image' ? item.kind === 'image'
                : item.kind !== 'other',
          )
  ...
  const renderGrid = () => {
    ...
    cardDisposers.clear()
    let grouped = state.group === 'folder'
    ...
    // is-grouped 必须反映**实际**渲染方式：分组回退成平铺时（这层没有子目录），
    // 平铺的分块布局和分组的卡片布局是两套 CSS，类挂错整个网格就散架。
  ...
  const renderMore = () => {
    ...
    const loadNext = async () => {
      ...
      recomputeMedia()
      renderSummary()
      if (state.scope === 'dir' && state.group === 'flat' && state.sort === 'new' && state.filter.trim() === '') {
