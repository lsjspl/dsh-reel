...
export function apply(ctx, entry = {}) {
   ...
   */
  const directoryEntriesOf = async (entry) => {
  ...
  const listingOf = async (scope, offset) => {
    const groups = []
    for (const entry of scope.entries) groups.push(await directoryEntriesOf(entry))
    const folders = groups.flatMap((group) => group.folders)
    ...
    return {
      ...
      parent: head === null || rel === '' ? null : keyFor(head.root.index, rel.split('/').slice(0, -1).join('/')),
      // 面包屑的契约恒定：crumbs[0] 是根、最后一级是当前目录，任何层都不少于一级。
      crumbs: head === null ? [{ key: LIBRARY_KEY, name: LIBRARY_LABEL }] : breadcrumbTrail(head.root, rel),
      folders,
      files: fileEntries,
      offset,
