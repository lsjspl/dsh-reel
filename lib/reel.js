 ...
 *    a configured directory; generated poster frames are cached in the OS temp
 *    directory, keyed by path + mtime + size.
...
const LIST_PAGE = 400
/** Poster frames are cached here, never inside a configured root. */
const THUMB_DIR = join(tmpdir(), 'dsh-reel-thumbs')
...
export function apply(ctx, entry = {}) {
  ...
  const listingOf = async (entryPoint, offset) => {
    ...
    return {
      ...
      crumbs: rel === '' ? [] : breadcrumbTrail(root, rel),
  ...
  const thumbnailFor = async (entryPoint) => {
    ...
    const cacheFile = join(THUMB_DIR, `${hash}.jpg`)
    const missFile = join(THUMB_DIR, `${hash}.miss`)
    try {
    ...
    const job = withThumbSlot(async () => {
      ...
      const scratch = join(THUMB_DIR, `${hash}.${process.pid}.part.jpg`)
  ...
  const previewFor = async (entryPoint) => {
    ...
    if (!info.isFile()) return null
    const hash = createHash('sha1').update(`${RENDER_VERSION}\0${entryPoint.abs}\0${info.size}\0${info.mtimeMs}`).digest('hex')
    const cacheFile = join(THUMB_DIR, `${hash}.webm`)
    const missFile = join(THUMB_DIR, `${hash}.preview-miss`)
    try {
      return await readFile(cacheFile)
    } catch {
      /* not cached yet */
    ...
    const job = withThumbSlot(async () => {
      ...
      const scratch = join(THUMB_DIR, `${hash}.${process.pid}.part.webm`)
      const offset = await middleOffset(entryPoint.abs)
      const ok = await runFfmpeg(binary, [
        '-hide_banner',
        '-loglevel', 'error',
  ...
  // GET /reel/api/browse?path=<dir> — folders only, for the settings picker.
  route(
    {
      kind: 'exact',
      path: `${BASE}/api/browse`,
      handler: guarded(async (req, res) => {
        if (req.method !== 'GET') return sendMethodNotAllowed(res, 'GET')
        const raw = urlOf(req).searchParams.get('path')
        if (raw === null || raw.trim() === '') {
          sendJson(res, 200, { path: '', parent: null, folders: systemRoots(), roots: roots(), configured: config().roots ?? [] })
          return
        }
        const target = resolve(String(raw).trim().replace(/[\\/]+$/, '') || sep)
        ...
        let rows
        try {
          rows = (await readDirectory(target)).filter((row) => row.isDirectory)
        } catch {
          sendError(res, 403, 'cannot read this directory')
          return
        }
        rows.sort((left, right) => left.name.localeCompare(right.name, 'zh-Hans-CN', { numeric: true }))
        const parent = dirname(target)
        sendJson(res, 200, {
          path: target,
          parent: samePath(parent, target) ? null : parent,
          folders: rows.map((row) => {
            const child = join(target, row.name)
            return {
              name: row.name,
              path: child,
              hidden: row.hidden,
              configured: roots().some((root) => samePath(root.path, child)),
            }
          }),
          roots: roots(),
          configured: config().roots ?? [],
        })
      }),
    },
    `GET ${BASE}/api/browse`,
  ...
  /** Windows drive letters, or the usual Unix mount points, as picker entry points. */
  const systemRoots = () => {
    ...
    return out.length > 0 ? out : [{ name: sep, path: sep }]
  }

  // GET/POST /reel/api/config — read and write the settings user layer.
  route(
    {
      kind: 'exact',
      path: `${BASE}/api/config`,
      handler: guarded(async (req, res) => {
        if (req.method === 'GET') {
          sendJson(res, 200, {
            writable: settingsScope !== null,
        ...
        if (settingsScope === null) {
          sendError(
            res,
            409,
            'no settings provider owns the reel namespace in this composition; put the directories in the reel row of cordis.patch.yml instead',
          )
          return
        }
        const body = await readJsonBody(req)
        if (typeof body !== 'object' || body === null || !Array.isArray(body.roots) || body.roots.some((item) => typeof item !== 'string')) {
          sendError(res, 400, 'body must be {"roots": ["<absolute directory>", ...]}')
          return
        }
        const cleaned = []
        for (const candidate of body.roots) {
          ...
          if (text === '') continue
          const abs = absolutize(text)
          if (abs === null) {
            sendError(res, 400, `not a usable directory: ${candidate}`)
            return
          }
          const info = await stat(abs).catch(() => null)
          if (info === null || !info.isDirectory()) {
            sendError(res, 400, `directory does not exist: ${abs}`)
            return
          }
          if (!cleaned.some((kept) => samePath(kept, abs))) cleaned.push(abs)
        }
        try {
          await settingsScope.update({ roots: cleaned })
        } catch (error) {
          sendError(res, 409, `the settings provider refused the write: ${error?.message ?? error}`)
          return
        }
        sendJson(res, 200, { ok: true, roots: roots(), revision: settingsRevision })
      }),
    },
    `GET/POST ${BASE}/api/config`,
  )
  ...
  route(
    {
      ...
      handler: guarded(async (req, res) => {
        ...
        const bytes = await thumbnailFor(entryPoint)
        if (bytes === null) {
  ...
  ctx.inject(['settings'], (settingsCtx) => {
    ...
    const entrySection = { roots: Array.isArray(entry.roots) ? entry.roots : [] }
    /**
   ...
   * exactly what schemastery emits for `z.object({ roots: z.array(z.string()) })`.
   *
  ...
  function rootsSchema() {
    ...
    schema.type = 'object'
    schema.dict = { roots: { sKey: 2 } }
    schema.meta = {}
    schema.toJSON = () => ({ uid: 1, refs })
    return schema
  }
