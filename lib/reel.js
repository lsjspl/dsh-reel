  ...
  '~standard': {
    ...
    validate(value) {
      ...
      if (raw.wallpaper !== undefined) {
        if (typeof raw.wallpaper !== 'string') issues.push({ message: 'wallpaper must be a string', path: ['wallpaper'] })
        else if (raw.wallpaper.trim() !== '') out.wallpaper = raw.wallpaper.trim()
      }
...
/**
 * 给依赖里那两个 CJS 二进制包用的同步 require：它们是 CJS，而这里是 ESM，
 * 动态 `import()` 是异步的，`findFfmpeg()` 是同步的——`createRequire` 正好补这个缝。
 */
...
/**
 * Keyframe interval for live transcoding, in frames, plus the matching minimum.
 *
 * `-movflags frag_keyframe` cuts a fragment at every keyframe, so a long GOP
 * would mean a long wait before the browser gets its first byte: x264's default
 * of 250 frames is ~8 seconds at 30fps. Fixing the GOP to about a second and
 * disabling scene-cut detection keeps the first fragment small and predictable.
 */
const TRANSCODE_GOP = 48
...
export function apply(ctx, entry = {}) {
  ...
  /**
   * The fullscreen wallpaper configured in the settings card.
   *
   * Two spellings are accepted: a media key (`r0/照片/a.jpg`) or an absolute
   * path that lives **inside a configured root**. Either way it is resolved
   * through the same helpers the cards use, so the wallpaper is served by
   * `/reel/stream` and `/reel/thumb` like any other entry: no new route, no
   * path outside the configured roots, nothing writable. A value that resolves
   * to nothing is reported once in the log and otherwise ignored — a typo must
   * not take the gallery down.
   *
   * @returns {Promise<object|null>} `{key, kind, name, url, poster}` or null.
   */
  let wallpaperComplained = ''
  const configuredWallpaper = async () => {
    const raw = config().wallpaper
    if (typeof raw !== 'string' || raw.trim() === '') return null
    const wanted = raw.trim()
    const complain = (reason) => {
      if (wallpaperComplained !== wanted) {
        wallpaperComplained = wanted
        log.warn('wallpaper %s ignored: %s', wanted, reason)
      }
      return null
    }
    ...
    let entry = null
    if (/^r\d+(?:\/|$)/.test(wanted)) {
      entry = await resolveKey(wanted)
      if (entry === null) return complain('no such media in the configured roots')
    } else {
      const abs = absolutize(wanted)
      if (abs === null) return complain('not a usable path')
      // 绝对路径也要落在某个配置根里：逐个根做 realpath 包含性验证，软链骗不过。
      for (const root of roots()) {
        const prefix = root.path.endsWith(sep) ? root.path : root.path + sep
        if (!abs.toLowerCase().startsWith(prefix.toLowerCase())) continue
        const rel = abs.slice(prefix.length)
        const contained = await resolveInside(root.path, rel)
        if (contained !== null) {
          entry = { root, abs: contained.abs, rel: contained.rel }
          break
        }
      }
      if (entry === null) return complain('outside every configured root')
    }

    let info
    try {
      info = await stat(entry.abs)
    } catch {
      return complain('cannot read it')
    }
    if (!info.isFile()) return complain('not a file')
    // 回整条条目（含 streamUrl / transcodeUrl / thumbUrl）：客户端用和卡片
    // **完全相同**的那套逻辑决定直连还是走转码流，服务端不替浏览器猜容器。
    return await fileEntry(entry.root, entry.rel, info)
  }
  ...
  /** The library key: one scope covering every configured root at once. */
  const LIBRARY_KEY = 'lib'
  /** Display name of the library scope, used in crumbs and feed labels. */
  const LIBRARY_LABEL = '库'

  /**
   * Resolve a key into the directory scope it addresses.
   *
   * Two kinds of keys exist. `r<index>/<rel>` addresses one directory inside
   * one configured root. `lib` addresses the whole library — every configured
   * root at once — which is what the tree's「库」node browses.
   *
   * Only the listing and scan endpoints speak this wider grammar. Endpoints
   * that address one file (`/stream`, `/thumb`, …) keep using resolveKey, so
   * `lib` can never resolve to a file.
   *
   * @param {unknown} raw - the key from a query string.
   * @returns {Promise<{library: boolean, entries: {root: object, rel: string, abs: string}[]}|null>} the scope, or null.
   */
  const scopeOf = async (raw) => {
    const key = normalizeKey(raw)
    if (key === LIBRARY_KEY) {
      return { library: true, entries: roots().map((root) => ({ root, rel: '', abs: root.path })) }
    }
    const entry = await resolveKey(key)
    return entry === null ? null : { library: false, entries: [entry] }
  }
  ...
  const fileEntry = async (root, rel, info) => {
    ...
    if (kind === 'video' || kind === 'audio') {
      ...
      // 浏览器不认的容器（AVI/WMV/RMVB…）走这条地址先转成 MP4。能直接播的条目
      // 也带着它：用不用由客户端按自己的解码能力决定，服务端不猜浏览器。
      if (ffmpegPath() !== null) {
        out.transcodeUrl = `${BASE}/transcode?k=${encodeURIComponent(key)}`
      }
  ...
  /**
   * One directory's folder and file rows.
   *
   * @param {{root: object, abs: string, rel: string}} entry - one directory.
   * @param {{tally?: boolean}} [options] - tally: 要不要顺手给每个子文件夹数一遍直接
   *   媒体数。库这一层用不到子文件夹（它的下一层是配置目录根），跳掉能省几十次
   *   readdir——库是落地页，这一层快一点是看得见的。
   * @returns {Promise<{folders: object[], files: {entry: object, row: object}[]}>} the rows.
   */
  const directoryEntriesOf = async (entry, options = {}) => {
    const { root, abs, rel } = entry
    ...
    if (options.tally !== false && folderEntries.length > 0 && folderEntries.length <= 400) {
      const tallies = await Promise.all(
        folderEntries.map((folder) => countMediaEntries(join(root.path, rel === '' ? folder.name : `${rel}/${folder.name}`))),
      )
      folderEntries.forEach((folder, index) => { folder.mediaCount = tallies[index].media })
    }
    return { folders: folderEntries, files: files.map((row) => ({ entry, row })) }
  }

  /**
   * List one scope: subdirectories plus its media files, media newest first.
   *
   * A scope is either one directory or the whole library. The library merges
   * the top level of every configured root into one list, sorted and paged as
   * if it were a single directory — so a page fetched after a scroll still
   * lines up with the `offset` the client holds.
   *
   * @param {{library: boolean, entries: object[]}} scope - a resolved scope.
   * @param {number} offset - page offset into the merged file list.
   * @returns {Promise<object>} the listing payload.
   */
  const listingOf = async (scope, offset) => {
    const groups = []
    for (const entry of scope.entries) groups.push(await directoryEntriesOf(entry, { tally: !scope.library }))
    // 库的直接子级是**配置的目录根**，和左树同构：库只有一个契约上的「下一层」，
    // 那就是各个根。早先把各根的下一层混在一起当成库的子文件夹，于是库的「▾」
    // 跳过了一层（列出一堆孙目录），面包屑也无从从库起步。
    const folders = scope.library
      ? await Promise.all(scope.entries.map(async (entry) => ({
        key: keyFor(entry.root.index, ''),
        name: entry.root.label,
        kind: 'directory',
        mediaCount: (await countMediaEntries(entry.root.path)).media,
      })))
      : groups.flatMap((group) => group.folders)
    const files = groups.flatMap((group) => group.files)
    folders.sort((left, right) => left.name.localeCompare(right.name, 'zh-Hans-CN', { numeric: true }))
    files.sort((left, right) => right.row.mtimeMs - left.row.mtimeMs || left.row.name.localeCompare(right.row.name))

    const page = files.slice(offset, offset + LIST_PAGE)
    const fileEntries = []
    for (const { entry, row } of page) {
      fileEntries.push(await fileEntry(entry.root, entry.rel === '' ? row.name : `${entry.rel}/${row.name}`, row))
    }
    const head = scope.library ? null : scope.entries[0]
    const rel = head === null ? '' : head.rel
    return {
      root: head === null
        ? { index: -1, path: '', label: LIBRARY_LABEL }
        : { index: head.root.index, path: head.root.path, label: head.root.label },
      rel,
      key: head === null ? LIBRARY_KEY : keyFor(head.root.index, rel),
      // 上一级：根之上是库（树里的父子关系就是这么画的），库里就没有上一级了。
      parent: head === null ? null : rel === '' ? LIBRARY_KEY : keyFor(head.root.index, rel.split('/').slice(0, -1).join('/')),
      // 面包屑的契约恒定：crumbs[0] 是库、crumbs[1] 是配置目录根、最后一级是当前
      // 目录，任何层都不少于两枚（库自己就是一枚）。路径行从库起步，才和左树
      // 的顶层是同一个东西。
      crumbs: head === null
        ? [{ key: LIBRARY_KEY, name: LIBRARY_LABEL }]
        : [{ key: LIBRARY_KEY, name: LIBRARY_LABEL }, ...breadcrumbTrail(head.root, rel)],
      folders,
      files: fileEntries,
      offset,
   ...
   * truncated answer rather than an unbounded one. A library scope seeds the
   * queue with every configured root; each entry's `rel` stays relative to the
   * root it belongs to, so the file keys it yields are exactly the single-root
   * ones (`r<index>/…`).
   *
   * @param {{library: boolean, entries: object[]}} scope - a resolved scope.
   ...
   */
  const scanOf = async (scope, options) => {
    ...
    const items = []
    const queue = scope.entries.map((entry) => ({ root: entry.root, abs: entry.abs, rel: entry.rel, depth: 0 }))
    let scanned = 0
    ...
    while (queue.length > 0) {
      ...
      for (const row of rows) {
        ...
        if (row.isDirectory) {
          if (!row.hidden) {
            queue.push({ root: current.root, abs: join(current.abs, row.name), rel: rowRel, depth: current.depth + 1 })
          }
          continue
        }
        if (!kinds.has(row.kind)) continue
        const entryStart = Date.now()
        items.push(await fileEntry(current.root, rowRel, { size: row.size, mtimeMs: row.mtimeMs }))
   ...
   */
  /**
   * 依赖里自带的二进制（`@ffmpeg-installer/ffmpeg` / `@ffprobe-installer/ffprobe`）。
   *
   * 这两个包的二进制在**平台子包的 tarball** 里，靠 optionalDependencies 按平台装，
   * 没有 postinstall 脚本——所以 pnpm ≥10 默认拦构建脚本也不影响它们。这里同步
   * require（CJS 包，`createRequire` 可以用），任何异常都只当作「没装」：平台没有
   * 对应子包、可选依赖被跳过、包被换掉，都不该让整个插件起不来。
   *
   * @param {string} spec - 包名。
   * @returns {string|null} 二进制绝对路径，或 null。
   */
  function npmBinary(spec) {
    try {
      const found = require(spec)
      return typeof found?.path === 'string' && existsSync(found.path) ? found.path : null
    } catch {
      return null
    }
  }
  ...
  function findFfmpeg() {
    // 插件自己的目录优先：`$DSH_HOME/reel/bin`。手动放一份进去就会被找到。
    const exeName = process.platform === 'win32' ? 'ffmpeg.exe' : 'ffmpeg'
    const ownBin = join(dshHome(), 'reel', 'bin', exeName)
    try {
      if (statSync(ownBin).isFile()) return ownBin
    } catch {
      /* 没装过 */
    }
    // 系统上已有的优先于依赖自带的：用户自己装的通常更新（winget/brew 的 6.x
    // 对依赖里那份 2021 年的 4.1）。依赖自带的只是兜底。
    if (hasOnPath('ffmpeg')) {
      return 'ffmpeg'
    }
    if (process.platform !== 'win32') {
      ...
      // Linux / macOS 的系统路径都没有时，用依赖自带的那份。
      return npmBinary('@ffmpeg-installer/ffmpeg')
    }
  ...
  const ffprobePath = () => {
    ...
    // 1) 优先用与所选 ffmpeg **同目录**的 ffprobe：版本对得上，`configured` 的
    //    ffmpegPath 指到哪，它的 ffprobe 就在旁边。
    ...
    // 2) 依赖里自带的 ffprobe（ffmpeg 来自 PATH 时就是这条兜底）。
    const bundled = npmBinary('@ffprobe-installer/ffprobe')
   ...
   * @param {number} [timeoutMs] - the deadline; posters are seconds, conversions are minutes.
   * @returns {Promise<boolean>} whether it exited zero in time.
   */
  const runFfmpeg = (binary, args, timeoutMs = FFMPEG_TIMEOUT_MS) =>
    new Promise((settle) => {
      let child
      try {
  ...
  /**
   * 边转边播：把一个 ffmpeg 的输出直接接到响应上。
   *
   * 浏览器不认的容器（AVI/WMV/RMVB…）没有缓存整片的余地——一部电影转一遍要
   * 好几分钟、还要占同样大的磁盘。这里改成「看哪转哪」：从 `start` 秒起编码，
   * fragmented MP4 边转边发，看到哪传到哪；拖动进度条就是换一个 `start` 重开
   * 一条流。CPU 只花在真正看过的部分上，磁盘一个字节都不占。
   *
   * 只有在 ffmpeg 真的吐出第一个数据块时才发 200：损坏的文件（ffmpeg 立刻
   * 失败）还能得到一句 503，而不是一个 200 配空响应体。请求断开（用户拖了
   * 进度条 / 关了页面）时杀掉 ffmpeg——不留没人看的编码进程。
   *
   * @param {import('node:http').ServerResponse} res - the response.
   * @param {string} binary - the ffmpeg executable.
   * @param {object} entryPoint - a resolved key.
   * @param {number} start - seconds to start from.
   * @param {Record<string, string>} headers - headers to send with the stream.
   * @returns {Promise<void>} resolves when the response is finished.
   */
  const streamTranscode = (res, binary, entryPoint, start, headers) =>
    new Promise((settle) => {
      let child
      try {
        child = spawn(binary, [
          '-hide_banner',
          '-loglevel', 'error',
          // 输入侧定位：跳到目标附近的关键帧再精确丢弃，不解码前面的内容。
          // 输出侧定位要先把前面整段解码扔掉，老片拖动会慢得不能接受。
          ...(start > 0 ? ['-ss', start.toFixed(3)] : []),
          '-i', entryPoint.abs,
          // 只带第一路视频和音频：老文件常有多音轨/字幕轨，浏览器不读它们。
          '-map', '0:v:0?',
          '-map', '0:a:0?',
          // H.264 + AAC 是每个现代浏览器都能解的少数组合之一；`veryfast` 让
          // 每次拖动后的等待尽量短，`yuv420p` 兜住老片源的 4:2:2/10bit 采样。
          '-c:v', 'libx264',
          '-preset', 'veryfast',
          '-crf', '23',
          '-pix_fmt', 'yuv420p',
          '-g', String(TRANSCODE_GOP),
          '-keyint_min', String(TRANSCODE_GOP),
          '-sc_threshold', '0',
          '-c:a', 'aac',
          '-b:a', '160k',
          '-ac', '2',
          // 分片 MP4：索引在前、每个关键帧一个分片，浏览器收到第一片就能播。
          '-movflags', 'frag_keyframe+empty_moov+default_base_moof',
          '-f', 'mp4',
          'pipe:1',
        ], { stdio: ['ignore', 'pipe', 'ignore'], windowsHide: true })
      } catch {
        sendError(res, 503, 'could not start the transcoder')
        settle()
        return
      }
      let started = false
      let finished = false
      /** 请求断开时别让 ffmpeg 继续编码（拖一次进度条就会断开一次）。 */
      const stop = () => {
        try {
          child.kill('SIGKILL')
        } catch {
      ...
      /**
       * 收尾：没开始就回一句可读的 503，已经发过 200 就正常结束流。
       *
       * @param {string} message - failure text for the not-started case.
       */
      ...
      child.stdout.on('data', (chunk) => {
        ...
        // `write` 返回 false 表示内核缓冲满了：暂停 ffmpeg 的输出，等 drain。
        if (res.write(chunk) === false) child.stdout.pause()
      })
      res.on('drain', () => child.stdout.resume())
      child.on('error', () => finish('could not start the transcoder'))
      child.on('close', (code) => finish(code === 0 ? 'nothing to convert' : 'conversion failed'))
    })
  ...
  route(
    {
      ...
      handler: guarded(async (req, res) => {
        ...
        sendJson(res, 200, {
          ...
          base: BASE,
          roots: roots(),
          // 设置里配的全屏壁纸（没配 / 解析不出来就是 null）。
          wallpaper: await configuredWallpaper(),
          ...
          capabilities: {
            ...
            // 浏览器不认的容器能不能转成 MP4 再播，取决于同一份 ffmpeg。
            transcode: ffmpegPath() !== null,
  ...
  // GET /reel/api/list?k=<key>&offset=<n> — one directory level, or `k=lib`
  // for the whole library (every configured root's top level, merged).
  route(
    {
      kind: 'exact',
      path: `${BASE}/api/list`,
      handler: guarded(async (req, res) => {
        if (req.method !== 'GET') return sendMethodNotAllowed(res, 'GET')
        const url = urlOf(req)
        const scope = await scopeOf(url.searchParams.get('k'))
        if (scope === null) {
          sendError(res, 404, 'unknown or unavailable directory')
          return
        }
        // 库的根们已经在 roots() 里验过存在；单目录这里再 stat 一次，
        // 因为目录可能在两次请求之间被删掉。
        if (!scope.library) {
          const info = await stat(scope.entries[0].abs).catch(() => null)
        ...
        sendJson(res, 200, await listingOf(scope, offset))
      }),
    },
    `GET ${BASE}/api/list`,
  )
  ...
  // `k=lib` walks every configured root in one pass.
  route(
    {
      kind: 'exact',
      path: `${BASE}/api/scan`,
  ...
  // GET /reel/transcode?k=<key>&t=<seconds> — 边转边播的 MP4。
  //
  // 浏览器不认的容器（AVI/WMV/RMVB…）在这里现转现看：ffmpeg 从 t 秒起编码，
  // fragmented MP4 直接管道给浏览器，看到哪转到哪、不落磁盘。拖动进度条由
  // 前端改成用新的 t 重开一条流（见 player.js 的 restartStream）。
  route(
    {
      kind: 'exact',
      path: `${BASE}/transcode`,
      handler: guarded(async (req, res) => {
        ...
        const kind = kindOf(entryPoint.abs)
        if (kind !== 'video' && kind !== 'audio') {
          sendError(res, 415, 'only video or audio can be converted')
          return
        }
        const binary = ffmpegPath()
        if (binary === null) {
          sendError(res, 503, 'no transcoder available')
          return
        }
        const headers = {
          'content-type': 'video/mp4',
          // 没有长度：这是一条边转边发的流；每次的 t 也不同，缓存没有意义。
        ...
        const start = Math.max(Number(url.searchParams.get('t') ?? '') || 0, 0)
        await streamTranscode(res, binary, entryPoint, start, headers)
      }),
    },
    `GET ${BASE}/transcode`,
  )
   ...
   * `z.object({ roots: z.array(z.string()).default([]), cacheDir: z.string(), wallpaper: z.string() })`
  ...
  function rootsSchema() {
    const refs = {
      ...
      3: { type: 'string', meta: {} },
      5: { type: 'string', meta: {} },
      4: { type: 'object', meta: { default: {} }, dict: { roots: 2, cacheDir: 3, wallpaper: 5 } },
    }
    const schema = (value) => {
      const raw = value === undefined || value === null ? {} : value
      ...
      for (const [key, item] of Object.entries(raw)) {
        if (key === 'roots' || key === 'cacheDir' || key === 'wallpaper') continue
        out[key] = item
      }
      ...
      if (raw.wallpaper !== undefined && raw.wallpaper !== null) {
        if (typeof raw.wallpaper !== 'string') throw new TypeError('$.wallpaper expected a string')
        if (raw.wallpaper.trim() !== '') out.wallpaper = raw.wallpaper.trim()
      }
      return out
    }
    schema.type = 'object'
    schema.dict = { roots: 2, cacheDir: 3, wallpaper: 5 }
    schema.meta = { default: {} }
